    ...
    <View key={device.name} style={styles.devices}>
      ...
      {action && (
        <View style={{ ...styles.device }}>
          <Button title={actionTitle} onPress={() => action(device)} color={Colors.accent2}/>
        </View>
      )}
    </View>
  ));
...
const PrinterSetupScreen = () => {
  ...
  const connectDevice = () => {
    NativeModules.RNZebraBluetoothPrinter.connectDevice(
      device.address
    ).then((res) => alert(res));
  };
    ...
    <SafeAreaView>
      <ScrollView>
        <View style={styles.container}>
          ...
          <Devices devices={devices} actionTitle="Print" action={printTestPage}/>
          <XButton title="View Unpaired Devices" onPress={viewUnpairedDevices} />
          {loadingUnpaired && <ActivityIndicator />}
          <Devices devices={unpairedDevices} actionTitle="connect" action={connectDevice} />
        </View>
      </ScrollView>
    </SafeAreaView>
  );
